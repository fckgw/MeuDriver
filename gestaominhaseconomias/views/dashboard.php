 ...
 * View: Dashboard Principal (PF) - COMPLETO E SINCRONIZADO
 */
...
$stmt_cat = $pdo->prepare("SELECT c.nome, SUM(m.valor) as total FROM minhaseconomias_movimentacoes m INNER JOIN minhaseconomias_categorias c ON m.categoria_id = c.id WHERE m.usuario_id = ? AND m.tipo = ? AND MONTH(m.data_vencimento) = ? AND YEAR(m.data_vencimento) = ? GROUP BY c.nome ORDER BY total DESC LIMIT 6");
    ...
    <div class="col-12 col-lg-9">
        <div class="card card-finance p-4 mb-4 border-0 shadow-sm bg-white">
            <form method="GET" action="index.php" class="row g-3 align-items-end">
                ...
                <div class="col-md-4"><label class="small fw-bold">Mês do Fluxo</label><select name="mes" class="form-select border-0 bg-light rounded-pill px-3 shadow-sm"><?php foreach($meses_nomes as $k=>$v) echo "<option value='$k' ".($mes_filtro==$k?'selected':'').">$v</option>"; ?></select></div>
                <div class="col-md-3"><label class="small fw-bold">Ano</label><select name="ano" class="form-select border-0 bg-light rounded-pill px-3 shadow-sm"><?php for($i=2024; $i<=2026; $i++) echo "<option value='$i' ".($ano_filtro==$i?'selected':'').">$i</option>"; ?></select></div>
                <div class="col-md-5"><button type="submit" class="btn btn-primary w-100 rounded-pill fw-bold">ATUALIZAR PAINEL</button></div>
            </form>
        </div>

        <!-- QUADRINHOS INDICADORES -->
        <div class="row g-3 mb-4">
            <div class="col-6 col-md-3"><div onclick="window.location.href='index.php?p=transacoes&f_tipo=Receita&f_status=Pago&mes=<?=$mes_filtro?>&ano=<?=$ano_filtro?>'" class="card card-indicator hover-receita p-3 bg-white shadow-sm text-center border-0"><small class="text-muted d-block fw-bold" style="font-size: 8px;">RECEBIDO</small><span class="text-success fw-bold">R$ <?= number_format($pago['rec'] ?? 0, 2, ',', '.') ?></span></div></div>
            <div class="col-6 col-md-3"><div onclick="window.location.href='index.php?p=transacoes&f_tipo=Despesa&f_status=Pago&mes=<?=$mes_filtro?>&ano=<?=$ano_filtro?>'" class="card card-indicator hover-pago p-3 bg-white shadow-sm text-center border-0"><small class="text-muted d-block fw-bold" style="font-size: 8px;">PAGO</small><span class="text-danger fw-bold">R$ <?= number_format($pago['desp'] ?? 0, 2, ',', '.') ?></span></div></div>
            <div class="col-6 col-md-3"><div onclick="window.location.href='index.php?p=transacoes&f_tipo=Receita&f_status=Futuro&mes=<?=$mes_filtro?>&ano=<?=$ano_filtro?>'" class="card card-indicator hover-receber p-3 bg-white shadow-sm text-center border-0"><small class="text-muted d-block fw-bold" style="font-size: 8px;">A RECEBER</small><span class="text-primary fw-bold">R$ <?= number_format($futuro['rec'] ?? 0, 2, ',', '.') ?></span></div></div>
            <div class="col-6 col-md-3"><div onclick="window.location.href='index.php?p=transacoes&f_tipo=Despesa&f_status=Futuro&mes=<?=$mes_filtro?>&ano=<?=$ano_filtro?>'" class="card card-indicator hover-pagar p-3 bg-white shadow-sm text-center border-0"><small class="text-muted d-block fw-bold" style="font-size: 8px;">A PAGAR</small><span class="text-warning fw-bold">R$ <?= number_format($futuro['desp'] ?? 0, 2, ',', '.') ?></span></div></div>
        </div>
        ...
        <div class="card card-finance p-4 bg-white shadow-sm border-0">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div class="btn-group btn-group-sm rounded-pill overflow-hidden border">
                    <a href="index.php?p=dashboard&mes=<?=$mes_filtro?>&ano=<?=$ano_filtro?>&cat_tipo=Despesa" class="btn <?= $tipo_cat_filtro == 'Despesa' ? 'btn-danger' : 'btn-light' ?>">Saídas</a>
                    <a href="index.php?p=dashboard&mes=<?=$mes_filtro?>&ano=<?=$ano_filtro?>&cat_tipo=Receita" class="btn <?= $tipo_cat_filtro == 'Receita' ? 'btn-success' : 'btn-light' ?>">Entradas</a>
                </div>
            </div>
...
<div class="modal fade" id="modalConta" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content border-0 shadow-lg" style="border-radius: 24px;"><div class="modal-header border-0 pt-4 px-4"><h5 class="modal-title fw-bold" id="labelModal">Gerenciar Conta</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><form action="index.php?mes=<?=$mes_filtro?>&ano=<?=$ano_filtro?>" method="POST"><input type="hidden" name="id_conta" id="id_conta"><div class="modal-body p-4"><div class="mb-3"><label class="small fw-bold">NOME</label><input type="text" name="nome" id="nome" class="form-control" required></div><div class="row g-3"><div class="col-md-6"><label class="small fw-bold">TIPO</label><select name="tipo" id="tipo" class="form-select"><option value="Carteira">Carteira</option><option value="Banco">Banco</option><option value="Cartao">Cartão</option><option value="Empresa">Empresa</option></select></div><div class="col-md-6"><label class="small fw-bold">SALDO INICIAL</label><input type="number" step="0.01" name="valor_inicial" id="valor" class="form-control" required></div></div><div class="mt-4 p-3 bg-light rounded-4 d-flex justify-content-between align-items-center"><span class="small fw-bold">CONTA ATIVA?</span><div class="form-check form-switch"><input class="form-check-input" type="checkbox" name="status" id="status" checked style="width: 2.5em; height: 1.25em;"></div></div></div><div class="modal-footer border-0 p-4"><button type="submit" name="btn_salvar_conta" class="btn btn-primary w-100 rounded-pill fw-bold py-3 shadow">SALVAR</button></div></form></div></div></div>

<!-- MODAL: EXCLUIR CONTA -->
<div class="modal fade" id="modalExcluir" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content border-0 shadow-lg rounded-4"><div class="modal-body p-5 text-center"><i class="fas fa-trash text-danger fa-4x mb-4 opacity-25"></i><h4 class="fw-bold text-dark">Excluir conta?</h4><p id="txtNomeExcluir" class="text-muted"></p><form action="index.php" method="POST"><input type="hidden" name="id_conta_excluir" id="id_conta_excluir"><div class="d-grid gap-2 mt-4"><button type="submit" name="btn_excluir_conta" class="btn btn-danger rounded-pill fw-bold py-2 shadow">SIM, EXCLUIR</button><button type="button" class="btn btn-light rounded-pill py-2" data-bs-dismiss="modal">CANCELAR</button></div></form></div></div></div></div>

<!-- MODAL: PENDÊNCIAS HOJE -->
<div class="modal fade" id="modalPendenciasHoje" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content border-0 shadow-lg rounded-4"><div class="modal-body p-5 text-center"><div class="mb-4"><i class="fas fa-bell text-warning fa-4x pulse-alert rounded-circle p-3" style="background: #fffcf0;"></i></div><h4 class="fw-bold text-dark">Lançamentos para Hoje</h4><p class="text-muted">Detectamos transações vencendo hoje em seu fluxo.</p><div class="d-grid gap-2 mt-4"><a href="index.php?p=transacoes&vencimento_hoje=1" class="btn btn-primary rounded-pill py-3 fw-bold shadow">VERIFICAR AGORA</a><button type="button" class="btn btn-light rounded-pill py-2 text-muted fw-bold" data-bs-dismiss="modal">Tratar depois</button></div></div></div></div></div>